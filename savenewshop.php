...
$db = mysqli_connect("mysql.foodiliciousbite.com", "biteuser", "Abs159..", "foodiliciousbite");
  if (isset($_POST['createshop'])) {
    ...
    $shop_contact= str_replace('-', '', $shop_contact);
    $shop_whatsapp = mysqli_real_escape_string($db, $_POST['shop_whatsapp']);
    $shop_whatsapp= str_replace(' ', '', $shop_whatsapp);
    $shop_whatsapp= str_replace('-', '', $shop_whatsapp);    
...
window.location.href='createshop.php';
	...
	$sql = "INSERT INTO t_shops (user_id,shop_name,shop_contact,shop_call,shop_whatsapp,shop_note,shop_town,shop_city,shop_country,shop_currency,creation_date) VALUES ('$user_id','$shop_name','$shop_contact','$shop_contact','$shop_whatsapp',  '$shop_note','$shop_town','$city_id','$country_id','$currency',CURDATE())";
...
window.location.href='createshop.php';
	...
	$sql = "INSERT INTO t_shops (user_id,shop_name,shop_contact,shop_call,shop_whatsapp,shop_note,shop_town,shop_city,shop_country,shop_lat,shop_long,shop_currency,creation_date) VALUES ('$user_id','$shop_name', '$shop_contact','$shop_contact','$shop_whatsapp','$shop_note','$shop_town','$city_id','$country_id','$shop_lat','$shop_long','$currency',CURDATE())";
	if($db->query($sql) == TRUE)
	{$product_sql=True;}
    else{
	 echo "<script>
...
window.location.href='createshop.php';
	...
	if($shop_image!="main_photo"){
	$target = "img/shop_images/".$last_id."_".basename($shop_image);
	}
	else{
	$target = "shop_images/main_photo";    
	}
	if($shop_image!="main_photo"){
	$image_temp= $last_id."_".$shop_image;
	}
	else
	{
	$image_temp="main_photo";
	}
	...
	$sql = "INSERT INTO t_images(shop_id,image_name,status) VALUES ('$last_id','$image_temp','NOT DELETED')";
	if($db->query($sql) == TRUE)
	{$product_sql=True;
	...
	header("Location: shops.php?shop_id=".$last_id."")  ;  
