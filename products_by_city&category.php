...
$_SESSION['url'] = $_SERVER['REQUEST_URI']."";
  ...
  // If upload button is clicked ...
   $rowperpage = 20;
			...
			$search_category = $searchwords[1];
                    
		  $query ="select P.product_id, (select  image_name from t_images where product_id = P.product_id limit 1) AS thumbnail, P.title,P.contact,P.details,P.tags,P.price, C.category_name,D.status,L.city,L.town,S.shop_name from products P left join t_category C on P.category=C.category_id JOIN t_locations L on P.product_id=L.product_id JOIN t_delivery_scope D on L.availability_rank=D.availability_rank  left join t_shop_contact SC on P.contact=SC.shop_contact  left join t_shops S on SC.shop_id=S.shop_id   where  ((L.city like '%".$search_city."%') OR (D.availability_rank='4')) AND (C.category_name like '%".$search_category."%')  ORDER BY P.product_id DESC LIMIT 80  ";
          //  $query = "SELECT P.product_id,P.title, (select  image_name from t_images where product_id = P.product_id limit 1) AS thumbnail, P.details, P.price, L.city,L.town, D.status from products P JOIN t_locations L On P.product_id=L.product_id JOIN t_delivery_scope D ON D.availability_rank=L.availability_rank where  (L.city like '%$search_city%') AND (P.details like '%$search_item%' OR P.title like '%$search_item%')   ORDER BY P.product_id DESC LIMIT 50  ";
...
<!-- Stylesheet
    ================================================== -->
<link rel="stylesheet" type="text/css"  href="css/productspagestyless_updated.css">
	    ...
	    <div class="container">
	      ...
	      <div class="collapse navbar-collapse" id="ftco-nav">
	        <ul class="navbar-nav ml-auto">
	          ...
	          <?php if ($_SESSION['login_user']!=null){echo "<li class='nav-item'><a href='' class='nav-link'>".$_SESSION['login_user']."</a></li>";}?>
	        </ul>
   ...
   //echo "<div id='restaurant-menu-background'>";		
    ...
    // echo "<div class='post'>"  ;      
	 //echo "<div class='menu-section' >";     
	  ...
	  if($tag!=null){
			echo "<div class='ribbon-wrapper'><div class='glow'>&nbsp;</div>";
